...
class TestTransformer(unittest.TestCase):
    ...
    def test_forward_pass(self):
        x = torch.randint(0, self.input_vocab_size, (self.batch_size, self.max_seq_len))
        y = torch.randint(0, self.target_vocab_size, (self.batch_size, self.max_seq_len))
        
        enc_padding_mask = create_padding_mask(x)
        dec_padding_mask = create_padding_mask(y)
        look_ahead_mask = create_look_ahead_mask(self.max_seq_len)
